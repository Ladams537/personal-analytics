...
router = APIRouter(
    prefix="/api/checkins", # Optional: Define prefix for all routes in this file
    tags=["Checkins"]       # Optional: Tag for API docs
)


@router.get("/today")
async def get_todays_checkin(current_user_id: uuid.UUID = Depends(get_current_user)):
    print(f"\n--- 1. get_todays_checkin called for user: {current_user_id} ---")
    today = date.today()
    conn = None
    cur = None
    try:
        ...
        # Fetch check-in specifically for today
        checkin_sql = """
        ...
        if not checkin_data:
            print(f"--- No check-in found for user {current_user_id} on {today} ---")
            # Return 404 Not Found if no check-in exists for today
        ...
        # Fetch completed routines (even though we don't save them yet, fetch if they exist)
        print(f"--- 4e. Fetching completed steps for check-in {checkin_id} ---")
        steps_sql = """
             SELECT cs.step_id, rs.step_name, cs.is_completed, cs.actual_duration
        ...
        full_checkin_details = {
            **dict(checkin_data),
            "metrics": metrics,
            "top_goal": top_goal,
            "completed_steps": completed_steps  # <-- Renamed from completed_routines
        }

        return full_checkin_details

    except HTTPException as e:
        # Re-raise HTTP exceptions (like the 404) so FastAPI handles them
        raise e
    except psycopg2.Error as db_error:
        print(f"DB Error: {db_error}")
        raise HTTPException(status_code=500, detail="Database error fetching today's check-in.")
...
@router.post("")
async def create_checkin(checkin_data: CheckinCreate, current_user_id: uuid.UUID = Depends(get_current_user)):
    ...
    try:
        ...
        # Fetch the generated checkin_id if needed, though we already have it
        # generated_checkin_id = cur.fetchone()[0]
        ...
        if checkin_data.metrics:
            ...
            """
            metrics_to_insert = [
                (uuid.uuid4(), checkin_id, metric.metric_type, metric.metric_name, metric.value)
                for metric in checkin_data.metrics
            ]
            # Use execute_batch for potentially better performance with many metrics
            psycopg2.extras.execute_batch(cur, metric_sql, metrics_to_insert)
            print(f"--- Inserted {len(metrics_to_insert)} metrics ---")

        # --- Insert into completed_routines ---
        if checkin_data.completed_steps:
            ...
            """
            steps_to_insert = [
                (uuid.uuid4(), checkin_id, step.step_id, step.is_completed, step.actual_duration)
        ...
        conn.commit()
        return {"status": "success", "message": "Check-in created successfully.", "checkin_id": checkin_id}

    except psycopg2.Error as db_error:
        print("\n--- !!! DATABASE ERROR !!! ---")
        # Print the whole error object and its type
        print(f"DB Error Type: {type(db_error).__name__}")
        print(f"DB Error Details: {db_error}") # Print the full error
        print(f"DB Error pgcode: {db_error.pgcode}") # Keep pgcode
        print(f"DB Error pgerror: {db_error.pgerror}") # Keep pgerror just in case
        if conn:
            print("--- Rolling back transaction ---")
            conn.rollback()
        # You might want to return a more generic error for security in production
        raise HTTPException(status_code=500, detail=f"Database error occurred. Code: {db_error.pgcode}")
